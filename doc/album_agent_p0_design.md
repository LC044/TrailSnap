...
## 23. P2.3 人物时光机（2026-09-05）

人物时光机把已归类的人脸照片整理为可解释的跨年人物时间线：

- 新增 `person-timeline` Skill 和只读 `get_person_timeline` 工具，返回人物的照片总量、拍摄时间跨度、年份分布、连续事件、常见地点、同框人物和代表照片；
- 人物详情页新增“人物时光机”入口，携带当前页面已确认的 `identity_id` 启动 Agent，避免仅凭姓名误选重名人物；
- 事件沿用最长 7 天、同日间隔超过 6 小时分段的规则；每个事件和年份只返回少量按回忆值、画面质量排序的代表照片；
- 单次最多加载 2000 张照片，并明确返回总数和截断状态；范围过大时 Agent 应引导用户按年份继续查看；
- 隐藏、删除或跨用户人物不可查询，隐藏人物和“未命名”人物不会出现在同行人统计中；同框只作为照片事实，不用于推断亲属、恋爱、职业或其他敏感关系；
- 用户确认需要保存后，可生成 `person_story` 结构化作品及 HTML；浏览和分析阶段不修改人物身份、人脸归属或照片。

真实 MiniMax-M3 验收对“周帅康”人物的 911 张照片完成工具调用，得到 2020-12-27 至 2025-12-05 的 6 年跨度、84 个候选事件和完整的年度分布。验收同时发现模型可能把真实媒体路径套在外部域名上，因此前端进一步实行媒体白名单：只接受可解析出真实 TrailSnap `photo_id` 的图片，并统一重写到当前 Server 地址，其他外部图片直接移除，不发起外部请求。
