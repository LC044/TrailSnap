 ...
 *
 * 拉起 tests/docker/docker-compose.e2e.yml：postgres + server(:8800) + frontend(:8082)。
 * p0 / smoke / all 套件依赖这套已启动的环境（playwright.config.ts 的 webServer 为 undefined）。
 * 跨平台替代 tests/scripts/e2e-up.ps1（PowerShell-only）。
 */
...
const composeFile = path.join(repoRoot, 'tests', 'docker', 'docker-compose.e2e.yml')
...
/** 轮询直到 url 返回 2xx-4xx（< 500），与 e2e-up.ps1 一致：后端根路径 404 也算就绪 */
