<template>
  <section class="mt-3 w-full overflow-hidden rounded-xl border border-primary-500/30 bg-slate-50 dark:bg-slate-900" aria-label="Agent 操作计划">
    <header class="flex items-start gap-3 p-3">
      <div class="rounded-lg bg-primary-500/10 p-2 text-primary-600"><Wrench v-if="isRepair" class="h-5 w-5" /><FolderPlus v-else class="h-5 w-5" /></div>
    ...
    <div class="border-t border-slate-200 px-3 py-3 dark:border-slate-700">
      <div v-if="isRepair" class="grid grid-cols-2 gap-2 text-xs sm:grid-cols-3">
        <div class="rounded-lg bg-white p-2 dark:bg-slate-800"><span class="block text-slate-500 dark:text-slate-400">操作</span><strong class="mt-0.5 block text-slate-800 dark:text-slate-100">结构修复</strong></div>
      ...
      <fieldset v-if="isRepair && repairs.length" class="mt-3 space-y-2" :disabled="current.status !== 'proposed' || busy">
      ...
      <div class="mt-3 flex flex-wrap items-center gap-2">
        <button v-if="current.status === 'proposed'" type="button" :disabled="busy || (isRepair && !selectedRepairIds.length)" class="inline-flex items-center gap-1.5 rounded-lg bg-primary-600 px-3 py-2 text-xs font-medium text-white hover:bg-primary-700 disabled:cursor-not-allowed disabled:opacity-60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2" @click="execute">
          <LoaderCircle v-if="busy" class="h-3.5 w-3.5 animate-spin" /><CircleCheck v-else class="h-3.5 w-3.5" />确认执行
        </button>
        <button v-if="current.status === 'proposed' && isRepair && selectionDirty" type="button" :disabled="busy || !selectedRepairIds.length" class="inline-flex items-center gap-1.5 rounded-lg border border-primary-500/40 px-3 py-2 text-xs text-primary-600 hover:bg-primary-500/10 disabled:cursor-not-allowed disabled:opacity-60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2" @click="saveSelection"><Save class="h-3.5 w-3.5" />保存选择</button>
        ...
        <button v-if="current.status === 'executed' && !isRepair" type="button" class="inline-flex items-center gap-1.5 rounded-lg border border-slate-300 px-3 py-2 text-xs text-slate-700 hover:bg-white dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2" @click="openAlbum"><ExternalLink class="h-3.5 w-3.5" />打开相册</button>
...
const execute = async () => {
  try {
    ...
    await ElMessageBox.confirm(message, isRepair.value ? '确认修复相册' : '确认执行相册整理', { type: 'warning', confirmButtonText: '确认执行', cancelButtonText: '取消' });
    busy.value = true;
    if (isRepair.value && selectionDirty.value) await persistSelection();
    const response: any = await agentApi.executeActionPlan(current.value.id);
    setCurrent(response.data);
    ElMessage.success(isRepair.value ? '相册结构修复完成，可随时撤销' : '相册整理完成，可随时撤销');
  } catch (error: any) {
