    ...
    <template v-else>
      ...
      <section class="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-800">
        <div class="flex flex-wrap items-center justify-between gap-3">
          ...
          <div class="flex gap-2">
            <button
              ...
              type="button"
              class="rounded-lg border border-gray-200 px-4 py-2 text-sm text-gray-700 hover:border-primary-500 hover:text-primary-600 dark:border-gray-700 dark:text-gray-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2"
            ...
            <button
              ...
              type="button"
              class="rounded-lg bg-primary-600 px-4 py-2 text-sm text-white shadow-primary-500/20 hover:bg-primary-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2"
        ...
        <div v-if="running || backedUp || skipped" class="mt-4 grid grid-cols-3 gap-3 text-center">
          ...
          <div class="rounded-lg bg-gray-50 p-3 dark:bg-gray-900">
            <p class="truncate text-lg font-semibold text-gray-700 dark:text-gray-200">{{ speedText }}</p>
            <p class="text-xs text-gray-500 dark:text-gray-400">当前速度</p>
          </div>
        </div>
        <p v-if="uploadedBytes > 0" class="mt-3 text-xs text-gray-500 dark:text-gray-400">
          已上传流量 {{ formatSize(uploadedBytes) }}，本轮媒体共 {{ formatSize(totalBytes) }}
        </p>
      </section>
...
const {
  ...
  overallProgress, lastError, lastRunAt,
} = backup
...
const statusText = computed(() => ({
  ...
  pausing: '正在暂停，等待当前分片完成…',
  paused: pauseReason.value === 'network' ? '等待 Wi-Fi / 可用网络' : '备份已暂停',
