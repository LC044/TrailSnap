...
def get_agent_tools(user_id: str, session_id: str | None = None) -> List[StructuredTool]:
    ...
    @tool
    def propose_album_metadata_repairs(
        album_id: Optional[str] = None,
        repair_ids: Optional[List[str]] = None,
        summary: Optional[str] = None,
    ) -> str:
        """为缺失 AI 视觉描述或文件指纹的照片生成待确认后台任务计划。

        repair_ids 使用 inspect_album_health 返回的 metadata_description 或 metadata_hash。
        本工具只生成预览；确认后任务在后台执行并可查看进度。不会删除或移动照片，
        但 AI 描述和文件指纹写入不可撤销，只能重新生成。
        """
        with SessionLocal() as db:
            try:
                row = propose_album_metadata_repair_plan(
                    db, user_id, session_id=session_id, album_id=album_id,
                    repair_ids=repair_ids, summary=summary,
                )
            except ValueError as exc:
