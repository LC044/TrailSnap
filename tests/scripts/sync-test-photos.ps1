...
.DESCRIPTION
    e2e 测试的 photos 夹具放在独立 Git 仓库 LC044/trailsnap-test-photos，
    以普通 git 跟踪（避免大量 jpg 进入主 repo）。本脚本把该仓库拉（克隆或更新）
    到 tests/fixtures/e2e-photos/，供 docker-compose.yml 挂载。

    行为：
      - 目标目录是空目录 → git clone
      - 目标目录是已有 git repo → git fetch + pull
      - 目标目录非空且不是 git repo → 报错退出

.PARAMETER RepoUrl
    远端 fixture 仓库 URL。默认从 $env:TS_TEST_PHOTOS_REPO 读取，否则用
    https://github.com/LC044/trailsnap-test-photos.git。
